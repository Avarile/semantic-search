...
async function initializeRedisClient() {
    ...
    if (useLocalRedis) {
        ...
        const localRedisPassword = process.env.LOCAL_REDIS_PASSWORD

        if (!localRedisUrl || !localRedisPassword) {
            console.error('Local Redis URL or Password not set')
            return null
        }

        redisClient = createClient({url: localRedisUrl, password: localRedisPassword})
        try {
...
const httpsAgent = new https.Agent({
    keepAlive: true,
    rejectUnauthorized: false // change to false if you want to ignore SSL certificate errors
